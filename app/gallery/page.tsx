    ...
    <div className="bg-slate-50 min-h-screen py-10 sm:py-14">
      <div className="w-full px-4 sm:px-8 lg:px-12 2xl:px-16 space-y-8">
        ...
        <div className="bg-white rounded-2xl border border-slate-200 p-4 sm:p-5 shadow-xs space-y-4">
          ...
          {/* Mobile Category Dropdown (No horizontal scrollbar) */}
          <div className="sm:hidden w-full pt-1">
            <div className="relative">
              <select
                value={selectedCategory}
                onChange={(e) => setSelectedCategory(e.target.value)}
                className="w-full appearance-none pl-4 pr-10 py-2.5 text-xs font-bold rounded-xl border border-slate-200 bg-slate-50 text-slate-800 focus:bg-white focus:outline-none focus:ring-2 focus:ring-red-600/20 focus:border-red-600 shadow-xs"
              >
                {CATEGORIES.map((cat) => (
          ...
          {/* Desktop Category Filter Chips (Wrap naturally, no scrollbar) */}
        ...
        {loading ? (
          <div className="bg-white rounded-2xl border border-slate-200 p-16 text-center space-y-3">
            ...
            <p className="text-xs font-semibold text-slate-600">Loading daily work gallery...</p>
          </div>
        ...
        {/* Lightbox / Video Modal Popup (Completely Transparent backdrop without dark overlays) */}
        {activeItem && (
          <div
            onClick={(e) => {
