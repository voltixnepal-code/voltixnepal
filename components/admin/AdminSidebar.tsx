...
export default function AdminSidebar({
    ...
    <div className="flex flex-col h-full bg-slate-900 text-slate-300">
      ...
      <div className="p-4 border-t border-slate-800 space-y-2">
        <Link
          href="/"
          target="_blank"
          className="flex items-center justify-between px-3 py-2 rounded-md text-xs font-medium text-slate-400 hover:bg-slate-800 hover:text-white transition-colors"
        >
        ...
        <button
          ...
          className="w-full flex items-center gap-2 px-3 py-2 rounded-md text-xs font-semibold text-red-400 hover:bg-red-950/40 hover:text-red-300 transition-colors"
        >
  ...
  return (
    <>
      ...
      <aside className="hidden md:block w-64 shrink-0 border-r border-slate-800 bg-slate-900 fixed inset-y-0 left-0 z-30">
        {sidebarContent}
      </aside>
      ...
      {mobileOpen && (
        <div className="md:hidden fixed inset-0 z-50 flex">
          <div
            ...
            onClick={onCloseMobile}
          />
          <div className="relative w-64 max-w-xs h-full bg-slate-900 z-10 shadow-2xl">
            {sidebarContent}
          </div>
        </div>
      )}
